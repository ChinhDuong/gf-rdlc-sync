...
namespace Mercury_v1_SureMedPlusRdlc.Views
{
        ...
        private void buildDataSubReport()
        {
            ...
            for (int i = 1; i <= 28; i++)
            {
                ...
                if (cup != null)
                {
                    ...
                    cupInfo.AbbrevDateStr = cup.IntakeDateTime.ToString("ddd") + " " + cup.IntakeDateTime.ToString("dd/MM");
            ...
            foreach (var item in listPillGroup.OrderBy(p => p.pillId))
            {
                if (item.pillId != null)
                {
                    ...
                    if (medicine.PillInfo != null)
                    {
                        ...
                        pillInside.DrugName = medicine.PillInfo.Shape;
                        pillInside.DrugName = medicine.PillInfo.Form;
                        pillInside.DrugName = medicine.PillInfo.Color;
                        pillInside.DrugName = medicine.PillInfo.Imprint;
                        pillInside.RefRemain = medicine.PrescriptionNumberOriginal;
