...
namespace SureMedPlusRdlc.Views
{
    public partial class RDLCReportView : INavigationAware, IRegionMemberLifetime, IActiveAware
    {
        ...
        void BindingDataToReport()
        {
            ...
            if (tray != null)
            {
                ...
                var cupLast = tray.Cups.OrderByDescending(x => x.IntakeDateTime).FirstOrDefault();
                var location = tray.Locations.FirstOrDefault().Value;
                var institutionLogo = location.Institution.InstitutionLogo;
                var customerAdrress = location.Customer.Addresses.FirstOrDefault(o => ("Medication packing address").Equals(o.AddressType));
                var institutionAdrress = location.Institution.Addresses.FirstOrDefault(o => ("Billing address").Equals(o.AddressType));
                ...
                var model = new SureMedRdlcReportOutsideInfo()
                {
                    ...
                    EndDate = cupLast.IntakeDateTime.ToString(defaultFormatDate),
                    ...
                    FacilityAndDayOfWeekStr = isMultiPatient ? Linguist.Phrase("ErrorDataTitle") : cupFirst.Location.Institution.Name + "/"+ cupFirst.IntakeDateTime.ToString("dddd"),
                    ...
                    FacilityStreetAddress = institutionAdrress != null ? buildFacilityStreetAddress(institutionAdrress) : "",
                    FacilityAddress = institutionAdrress != null ? buildFacilityAddress(institutionAdrress) : "",
                    BarcodeImage = Utils.CreateQRCode(labelId),
                    FacilityLogo = institutionLogo != null ? Utils.imageToByteArray(institutionLogo) : new byte[0],
                    ...
                    PatientZipCode = patient.Addresses.FirstOrDefault() != null ? patient.Addresses.FirstOrDefault().City + ", " + patient.Addresses.FirstOrDefault().StateProvince + " " + patient.Addresses.FirstOrDefault().ZipPostalCode : "",
                    PatientPhone = patient.Phone1 != null ? patient.Phone1 : ""
                };
        ...
        private void buildDataSubReport()
        {
            ...
            for (int i = 1; i <= 28; i++)
            {
                ...
                if (cup != null)
                {
                    ...
                    cupInfo.IntakeDateTimeStr = (isDoesDispen ? cup.TimeSlotDescription : cup.IntakeTimeDescription) + " " + cup.IntakeDateTime.ToString(defaultFormatDate);
                    ...
                    cupInfo.DayName = cup.IntakeDateTime.ToString("dddd");
        ...
        private void localReport_SubreportProcessing(object sender, SubreportProcessingEventArgs e)
        {
            ...
            if (e.ReportPath == "drugDetail" || e.ReportPath == "drugDetailCol1" || e.ReportPath == "drugDetailCol2")
            {
                ...
                if (pillInfoList.Count() <= 14)
                {
                    for (int i = pillInfoList.Count(); i <= 14; i++)
                    {
            ...
            if (e.ReportPath == "page2")
            {
                ...
                e.DataSources.Add(new ReportDataSource("PillInfoPage2DataSet", listPillInside.Take(14)));
            }
            #endregion
        }
        ...
        private string buildFacilityStreetAddress(AddressData institutionAddress)
        {
            var result = "";
            result = result + (string.IsNullOrEmpty(institutionAddress.Street) ? "" : institutionAddress.Street + Environment.NewLine);
            return result;
        }
        private string buildFacilityAddress(AddressData institutionAddress)
        {
            var result = "";
            result = result + (string.IsNullOrEmpty(institutionAddress.City) ? "" : institutionAddress.City + (string.IsNullOrEmpty(institutionAddress.StateProvince) ? "" : ", " + institutionAddress.StateProvince) + (string.IsNullOrEmpty(institutionAddress.ZipPostalCode) ? "" : " " + institutionAddress.ZipPostalCode) + Environment.NewLine);
            return result;
        }
